...
if not VREnabled then
	MainSection:NewButton("Get VR Players", "notify VR Players", function()
		for i, v in pairs(game:GetService("Workspace").VR:GetChildren()) do
	...
	MainSection:NewToggle("Disable VR Hands Collison", "VR People can hit u to space anymore lol", function(state)
		for i, v in pairs(game:GetService("Workspace").VR:GetDescendants()) do
			if v:IsA("Model") and v.Name == "HandL" or v.Name == "HandR" then
		...
		task.spawn(function()
			while true do
				...
				for _, part in ipairs(parts) do
					if (part.Name == "Hand") and (part.Parent.Name == "HandL" or part.Parent.Name == "HandR") then
	...
	MainSection:NewToggle("annoy VR User", "loop teleports to a vr user and block their vision use fly with this in iy or sum", function(state)
		...
		task.spawn(function()
			while true do
				...
				for i, v in pairs(targchar:GetChildren()) do
					if v.Name == "Head" then
						game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame = annoy.CFrame * CFrame.new(0, 2, -24)
					end
				end
			end
		end)
	...
	MainSection:NewToggle("weld to VR User hand", "weld to a vr players hand", function(state)
		...
		for i, v in pairs(targchar:GetDescendants()) do
			if v.Name == "Hand" and v.Parent.Name == "HandR" then
				annoy = v
			end
		end
		task.spawn(function()
	...
	MainSection:NewButton("FE Bring all props", "loop teleports all props to localplayer NOTE MOST OF THEM ARE NOT VISIBLE TO OTHERS BUT SOME ARE AND IF U SPAM IT MORE WILL BE FE", function()
        ...
        for i, v in pairs(game:GetService("Workspace").Items:GetChildren()) do
			if v:IsA("Model") then
				for _, prop in pairs(v:GetDescendants()) do
					if prop:IsA("Part") or prop:IsA("MeshPart") or prop:IsA("BasePart") then
						prop.CFrame = ltargchar.HandR:WaitForChild("Hand").CFrame
					end
				end
			end
		end
