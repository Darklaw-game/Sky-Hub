...
MainSection:NewButton("Auto Farm by lua_u", "auto farm wads", function()
	task.spawn(function()
		while task.wait(0.2) do
			for i, v in pairs(game:GetService("Workspace").WorldObjects:GetDescendants()) do
				if v.Name == "wad" then
					...
					game:GetService("ReplicatedStorage").Networking.NetworkingEvent:FireServer(unpack(args))
				end
			end
		end
	end)
	local function WaitForDescendant(Parent, Child, TimeOut)
	...
	while task.wait() do
		...
		local objects = game:GetService("Workspace").WorldObjects:GetDescendants()
		for i,v in pairs(objects) do
			if v:IsA("ObjectValue") and v.Name == "SittingModel" and v.Value == char then
				char = v.Parent.Parent
			end
		end
		char:PivotTo(CFrame.new(11010,142,0))
		local temp = WaitForDescendant(game:GetService("Workspace"), "PunchingHazard").Parent
		task.wait(0.5)
		char:PivotTo(temp.Chest.CFrame)
		task.wait(0.5)
		char:PivotTo(CFrame.new(10990,142,0))
		task.wait(1.5)
	end
end)
